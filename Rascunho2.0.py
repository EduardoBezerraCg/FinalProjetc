...
class InterfaceGrafica:
    def __init__(self, janela, db_file='frota.db'):
        ...
        btn_listar_veiculos_indisponiveis = ctk.CTkButton(self.janela, text="Indisponíveis",
                                                          command=lambda: self.get_veiculos_disp(0))
        btn_listar_veiculos_indisponiveis.grid(row=2, column=0, columnspan=3)

        btn_editar_vhc = ctk.CTkButton(self.janela, text='Editar VHC', command=lambda: self.editar_veiculo)
    ...
    def verificar_proxima_legalizacao(self, nome_veiculo):
        try:
            ...
            if diferenca <= limite_alerta_3:
                while True:
                    messagebox.showerror("Data Limite",
                                         f"Data Limite para legalizar o veiculo {nome_veiculo}, apenas {days_legal} para o mesmo")
                    self.resposta = messagebox.askyesno(title="Legalizacao",
                                                        message=f'O veículo {nome_veiculo} está muito perto da data de legalização, restam {days_legal} dias\n\nDeseja colocar o veículo para legalizar?')
                    ...
                    if self.resposta:
                        ...
                        messagebox.showwarning('Aviso',
                                               f'Conduzir com um veículo não legalizado é passível de apreensão, favor legalizar, restam {days_legal} dias')
                        self.resposta = messagebox.askyesno(title="Legalizacao",
                                                            message=f'Veículo {nome_veiculo} perto da data de legalização\n\nDeseja colocar o veículo para legalizar?')
                ...
                messagebox.showwarning("Alerta",
                                       f"O veículo {nome_veiculo} está à '{days_legal}' dias próximo da data de legalização!, ter atenção às datas")
                self.resposta = messagebox.askyesno(title="Legalizacao",
                                                    message=f'Veículo {nome_veiculo} perto da data de legalização, restam {days_legal} dias;\nDeseja colocar o veículo para legalizar?')
                if self.resposta:
                    self.legalizar_veiculo_db(nome_veiculo)
                else:
                    showinfo('Aviso',
                             f'Conduzir com um veículo não legalizado é passivel de apreensão, favor legalizar\nTer atenção às datas, restam apenas {days_legal} dias para legalizar o veículo')
            ...
            #     messagebox.showinfo("Informação", f"O veículo {nome_veiculo} está regular")
                ...
                self.resposta = messagebox.askyesno(title="Legalizacao",
                                                    message=f'Veículo {nome_veiculo} à {days_legal} dias perto da data de legalização\nDeseja colocar o veículo para manutenção?')
                if self.resposta:
                    self.legalizar_veiculo_db(nome_veiculo)
                else:
                    showinfo('Aviso',
                             f'Conduzir com um veículo não legalizado é passivel de apreensão, faltam {days_legal} dias para legalização, favor legalizar')
                    self.get_veiculos_disp(0)
