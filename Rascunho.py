...
class InterfaceGrafica:
    def __init__(self, janela, db_file='frota.db'):
        ...
        btn_listar_veiculos_disponiveis = tk.Button(
            self.janela, text="Disponíveis", font="Arial 11", command=lambda: self.get_veiculos_disp(1))
        btn_listar_veiculos_disponiveis.grid(row=1, column=0, columnspan=2)

        btn_listar_veiculos_indisponiveis = tk.Button(
            self.janela, text="Indisponíveis", font="Arial 11", command=lambda: self.get_veiculos_disp(0))
        btn_listar_veiculos_indisponiveis.grid(row=2, column=0, columnspan=2)

        # Botões do Rodapé
        frame_rodape = ttk.Frame(self.janela)
        frame_rodape.grid(row=10, column=0, columnspan=2, pady=10, sticky=tk.W)

        # Botões do Rodapé
        btn_revisao = ttk.Button(
            frame_rodape, text='Manutenção'.upper(), command=self.revisao)
        btn_revisao.grid(row=0, column=0, padx=5, pady=5)

        btn_legalizar = ttk.Button(
            frame_rodape, text='ESTADO LEGAL', command=self.estado_legal)
        btn_legalizar.grid(row=0, column=1, padx=5, pady=5)

        btn_auxiliar = ttk.Button(
            frame_rodape, text='TIRAR DA MANUTENÇÃO)', command=self.func_btn_auxiliar)
        btn_auxiliar.grid(row=0, column=2, padx=5, pady=5)

        btn_teste_compra_veiculos = ttk.Button(frame_rodape, text='COMPRAR VHC', command=self.adicionar_veiculo)
    ...
    def adicionar_veiculo(self):
        ...
        largura = 300
        altura = 300
        janela_adicionar.geometry(f"{largura}x{altura}")

        # Criar widgets na nova janela
        lbl_modelo = ttk.Label(janela_adicionar, text="Modelo:", font=('Calibri', 11))
        lbl_modelo.grid(row=0, column=0, padx=10, pady=5, sticky=tk.W)

        entry_modelo = ttk.Entry(janela_adicionar)
        entry_modelo.grid(row=0, column=1, padx=10, pady=5, sticky=tk.W)

        # Campo Tipo
        lbl_tipo = ttk.Label(janela_adicionar, text="Tipo: ", font=('Calibri', 11))
        ...
        lbl_data_revisao = ttk.Label(janela_adicionar, text="Data de Revisão\n(Dia/Mes/Ano):", font=('Calibri', 11))
        lbl_data_revisao.grid(row=2, column=0, padx=10, pady=5, sticky=tk.W)

        entry_data_revisao = ttk.Entry(janela_adicionar)
        entry_data_revisao.grid(row=2, column=1, padx=10, pady=5, sticky=tk.W)

        # Adicionar campo para a categoria
        lbl_categoria = ttk.Label(janela_adicionar, text="Categoria:", font=('Calibri', 11))
        ...
        lbl_data_legalizacao = ttk.Label(janela_adicionar, text="Data da Legalização\n(Dia/Mes/Ano):",
                                         font=('Calibri', 11))
        lbl_data_legalizacao.grid(row=4, column=0, padx=10, pady=5, sticky=tk.W)

        entry_data_legalizacao = ttk.Entry(janela_adicionar)
        entry_data_legalizacao.grid(row=4, column=1, padx=10, pady=5, sticky=tk.W)

        # Criando o botão
        btn_salvar = ttk.Button(janela_adicionar, text="Salvar", command=lambda: self.salvar_veiculo(
    ...
    def salvar_veiculo(self, modelo, data_revisao, tipo, categoria, data_legalizacao, janela_adicionar):
        try:
            ...
            # Converter a data de revisão para o formato desejado (por exemplo, "%Y/%m/%d")
            ...
            # Atualizar a tabela com os veículos disponíveis
        ...
        try:
            if quantidade_item <= 1:
                messagebox.showerror("Estoque Zerado",
                                     f"Nosso estoque tinha apenas {quantidade_item} veiculos disponiveis;\nnos restam 0")
                self.resposta = messagebox.askyesno(title="Estoque baixo", message=f'Deseja comprar mais veíuculos?')
                if self.resposta:
                    self.adicionar_veiculo()
                else:
                    self.get_veiculos_disp(0)
                showinfo("Aviso", "Será encaminhado para os veículos indisponíveis")


            elif quantidade_item <= 3:
                messagebox.showerror("Estoque Em Nível Critico",
                                     f"Nosso estoque tem apenas {quantidade_item} veiculos disponiveis")

            elif quantidade_item == 5:
                messagebox.showwarning(title=f"Estoque Baixo",
                                       message=f'O estoque está baixando, restam apenas {quantidade_item} veiculos')
    ...
    def manutencao_veiculo(self):
        ...
        try:
            # Obtém o ID do veículo selecionado na tabela
    ...
    def converter_para_datetime(self, data_str):
        ...
        # Se não for possível, trate conforme necessário (pode retornar None ou levantar uma exceção)
        return None
    ...
    class InterfaceGrafica:
        ...
        def modif_vhc(self):
            try:
                ...
                # Consulta SQL modificada para incluir a categoria
                query = "SELECT Categoria FROM Veiculos WHERE Modelo = ?"
                cursor = self.db_consulta(query, (nome_veiculo,))
                categoria_veiculo = cursor.fetchone()

                if not categoria_veiculo:
                    messagebox.showinfo("Aviso",
                                        f"Não foi possível encontrar detalhes para o veículo {nome_veiculo}, categoria desconhecida.")
                    return

                messagebox.showinfo("Aviso",
                                    f"Não foi possível encontrar detalhes para o veículo {nome_veiculo}, categoria {categoria_veiculo[0]}.")
